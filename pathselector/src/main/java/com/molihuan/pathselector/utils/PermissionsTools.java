...
public class PermissionsTools {
     ...
     */
    public static void getStoragePermissions(Fragment fragment, OnPermissionCallback specialPermissionCallback,
                                             OnPermissionCallback generalPermissionCallback) {

        //所有文件访问权限Android 11 +
        specialPermissionsOfStorage(fragment, specialPermissionCallback);

        //普通存储访问权限Android 11 -
        generalPermissionsOfStorage(fragment, generalPermissionCallback);
     ...
     */
    public static void specialPermissionsOfStorageWithDialog(Fragment fragment, boolean useDialog,
                                                             OnPermissionCallback specialPermissionCallback) {
        if (useDialog) {
            ...
            boolean isGet = PermissionX.isGranted(fragment.getContext(), Manifest.permission.MANAGE_EXTERNAL_STORAGE);
            //已有权限则返回
            if (isGet) {
            ...
            new MessageDialog(fragment.getContext())
                    .setContent(new FontBean(fragment.getString(R.string.tip_dialog_get_special_permissions)))
                    .setConfirm(new FontBean(fragment.getString(R.string.option_confirm)), new BaseDialog.IOnConfirmListener() {
                        @Override
                        public boolean onClick(View v, BaseDialog dialog) {
                            PermissionsTools.specialPermissionsOfStorageNoCheck(fragment, specialPermissionCallback);
                            dialog.dismiss();
                            return false;
                        }
                    })
                    .setCancel(new FontBean(fragment.getString(R.string.option_cancel)), new BaseDialog.IOnCancelListener() {
                        @Override
                        public boolean onClick(View v, BaseDialog dialog) {
                            dialog.dismiss();
                            return false;
     ...
     */
    public static void generalPermissionsOfStorage(Fragment fragment, OnPermissionCallback generalPermissionCallback) {

        boolean isGet = PermissionX.isGranted(fragment.getContext(), Manifest.permission.WRITE_EXTERNAL_STORAGE);
        //已有权限则返回
        if (isGet) {
        ...
        PermissionX.init(fragment)
                ...
                .onExplainRequestReason((scope, deniedList) -> scope.showRequestReasonDialog(deniedList,
                        fragment.getResources().getString( R.string.tip_permission_failed),
                        "OK", "Cancel"))
                .request((allGranted, grantedList, deniedList) -> {
                    if (allGranted) {
                        generalPermissionCallback.onGranted(grantedList, allGranted);
                    } else {
                        generalPermissionCallback.onDenied(deniedList, allGranted);
                    }
                });
    }
     ...
     * @param specialPermissionCallback 权限申请结果回调
     */
    public static void specialPermissionsOfStorage(Fragment fragment, OnPermissionCallback specialPermissionCallback) {
        if (!VersionTool.isAndroid11()) {
            return;
        }
        ...
        specialPermissionsOfStorageNoCheck(fragment, specialPermissionCallback);

    }

    public static void specialPermissionsOfStorageNoCheck(Fragment fragment, OnPermissionCallback specialPermissionCallback) {
        PermissionX.init(fragment)
                .permissions(new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE, Manifest.permission.READ_EXTERNAL_STORAGE})
                .explainReasonBeforeRequest()
                .onExplainRequestReason((scope, deniedList) -> scope.showRequestReasonDialog(deniedList,
                ...
                .request((allGranted, grantedList, deniedList) -> {
                    if (allGranted) {
                        specialPermissionCallback.onGranted(grantedList, allGranted);
                    } else {
                        specialPermissionCallback.onDenied(deniedList, allGranted);
                    }
                });
    }
