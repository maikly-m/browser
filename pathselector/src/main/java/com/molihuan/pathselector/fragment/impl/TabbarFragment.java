...
public class TabbarFragment extends AbstractTabbarFragment implements View.OnClickListener, OnItemClickListener {
    ...
    public void initView() {
        ...
        tabbarListAdapter = new TabbarListAdapter(R.layout.item_tabbar, tabbarList);//适配器添加数据
    ...
    public void onItemClick(@NonNull BaseQuickAdapter<?, ?> adapter, @NonNull View v, int position) {
        if (adapter instanceof TabbarListAdapter) {
            ...
            if (path.length() <= MConstants.DEFAULT_ROOTPATH.length() && (!MConstants.DEFAULT_ROOTPATH.equals(path))) {
                Mtools.toast(String.format(getString(R.string.tips_path_jump_error_exceeds_default_path), path, MConstants.DEFAULT_ROOTPATH));
